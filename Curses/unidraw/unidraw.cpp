...
void draw_line2(Canvas& canvas, int x1, int y1, int x2, int y2){
	int e = x2 - x1;
	int dx = e * 2;
	int dy = (y2 - y1) * 2;
...
void Turtle::draw(float distance){
	...
	Vector2i origin(position);

	move(distance);

	draw_line(*m_canvas, origin, position);
}

void Turtle::move(float distance){

		float radian = (rotation * M_PI) / float(180);
		
		position = Vector2i(distance * cos(radian), distance * sin(radian));
}
